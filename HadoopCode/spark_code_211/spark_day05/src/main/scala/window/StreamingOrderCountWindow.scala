...
object StreamingOrderCountWindow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 4、对获取的数据进行ETL处理——————————+++++++++++++++++++++++++++++++%%%%%%%%%%%%%%%%
    val orderCountDStream: DStream[(Int, Long)] = windowDStream.transform{ rdd =>
    ...
    // 6、对于流式应用来说，需要启动应用，正常情况下启动以后一直运行，直到程序异常终止或者人为干涉
