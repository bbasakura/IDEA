...
object AnalyzeRequest {
  ...
  /**
    * 不同类型数据，根据不同解析规则进行解析
    */
  def analyzeQueryRequest(requestTypeLabel: RequestType,//请求类型标签,比如国内查询
                          requestMethod: String,//请求方式,比如get/post
                          contentType: String,//请求类型
                          request: String,//请求的URL
                          requestBody: String,//请求体数据,所有的请求信息都在这儿呢.
                          travelType: TravelTypeEnum,//往返类型
                          analyzeRules: List[AnalyzeRule]//数据库中的解析规则
    ...
    if (matchedRules.size > 0) {
      ...
      if (matchedRule.requestMethod.equalsIgnoreCase("GET") && matchedRule.isNormalGet) {
        //如果解析Get请求参数.
//        https://b2c.csair.com/B2C40/newTrips/static/main/page/booking/index.html?t=R&c1=CTU&c2=SHA&d1=2019-10-08&d2=2019-11-14&at=1&ct=0&it=0
        //先用?号切,取后面的.
        //用&号切.获取到参数键值对.
        //用=号,后面的就是具体的参数值.
        //c1 => CTU  //目的地.
