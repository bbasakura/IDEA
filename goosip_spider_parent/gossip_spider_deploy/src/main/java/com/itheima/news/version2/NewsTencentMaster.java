 ...
 * @Description: 获取news新闻数据,(还需要判断url是否已经爬取过)
 *                  将获取到的news数据保存到redis的list队列中(bigData:spider:newsJsonList)
 * @date 2019/7/110:26
 */
...
//            saveNewsListToDb(nohotNewsList);
    ...
    /**
     * 将新闻数据保存到redis的list队列中
     * @param nohotNewsList
     */
    private static void saveNewsToRedis(List<News> nohotNewsList) {
        for (News news : nohotNewsList) {
            //将对象转换成json数据
